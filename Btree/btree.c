...
Data* search_recursive(Btree* tree, int key, BNode* root) {
    ...
    if(i < root->n && key == root->records[i].key)
        return &root->records[i];//  great if you found the key at this node , else check if the given node is leaf else go to the approprotate child
        ...
        BNode* c_i = (BNode*)malloc(sizeof(BNode));
        read_file(tree, c_i, root->children[i]);// move to the child and find 
